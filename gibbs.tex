...
On the other hand, the insoluble cross-linked hydrogel network enables dewatering under the influence of external (thermal and/or mechanical) stimulus.
Moreover, from a thermodynamic perspective, the polyelectrolyte hydrogel is already an osmotic membrane.
These above mentioned hydrogel properties designates them as excellent candidates for desalination, and, at the same time, avoid the use of expensive membranes.
In this article, we present our recent theoretical study of polyelectrolyte hydrogel usage for water desalination. 
Employing coarse-grained model and the Gibbs ensemble, we modeled the thermodynamic equilibrium between coexisting gel phase and supernate aqueous salt solution phase.
We performed a sequence of step-by-step hydrogel compressions in \emph{open} and \emph{closed} systems.
The compression in \emph{open system} causes the decrease of salt ions inside the hydrogel,
while the compression in \emph{closed system} causes the decrease of the salt concentration in the supernatant solution.
We modeled this stepwise process of continuous decrease of water salinity from seawater up to freshwater concentrations.
We estimated the energy costs of the entire process as 38.2 J per production of 1 liter of potable water, which is comparable with reverse osmosis.
}
...
Waste water treatment and technology is one of the greatest concerns of modern sustainable society. 
The water has to be disposed of both biological \cite{guesmi2022} and chemical \cite{baaloudj2022_1,shahzad2022} pollutants.
Most imporatntly, water treatment technologies have to be prepared for the still increasing need for production of potable water from the brine, \ie for desalination.
...
TODO Then, highlight the water desalination technologies and Gibbs ensemble (https://doi.org/10.3390/nano10020293 \cite{erdos2020}).
...
We propose a cascade desalination process to lower supernatant salinity.
%it is worth to note that we are not limited with this process only.
For this process the gel must be compressed/decompressed in an \emph{open} and \emph{closed} system.
By the \emph{open system} we mean that the gel is in thermodynamic equilibrium with huge, effectively \textbf{infinite}, reservoir of an aqueous solution. 
%The gel freely exchanges ions with the reservoir, while their concentration in reservoir (and chemical potential) remains constant.
...
By the \emph{closed system} we mean that the gel is in equilibrium with a \textbf{finite} reservoir of an aqueous solution. 
In this case the exchange of ions between the gel and the reservoir may affect their concentration in reservoir.%, whereas the  
%again have the same ions chemical potential, the same temperature, and desired pressure difference between the gel and the reservoir. 
%Although, the number of ions and volume of gel and reservoir may vary, 
%total number of $\na$ and $\cl$ ions in the gel and in the reservoir, \ie in the volume $\Vbox$ remains constant.

Due to huge size of the reservoir, the compression of the gel in the \emph{open system} does not affect the reservoir salinity, $\cs =$ $\cna=$ $\ccl=$ $\text{Const}$ (\reffig{fig: open}), whereas the number of ions entrapped in the volume $\Vbox$ changes (\reffig{fig: closed}).
...
%In both cases there are two coexisting systems with constant $\mu P T$ allowed to exchange particles and change volume, which is represented as Gibbs ensemble.
...
The mechanical movement and the exchange of ions occur simulataneously in real systems, however, we simulate them alternating each other in stop-run mode in a random order. 
...
The details of this hybryd computational technique can be found in our previous studies of polyelectrolytes in open system \cite{Rud2020, Rud2022, Landsgesel2020a}. 
 
%In this sense employed techniques is called MCMD. 
...
of $\na$ ions is bigger than that of $\cl$ by the number of hydrogel units, $\Ngel$.
The solvent assumed to be water, which is modeled implicitly via setting up dielectric permittivity $\epsilon=80$.
...
The non-bonded interactions are modelled by the truncated Lennard-Jones interaction potential.
...
\end{equation}
where $\lb$ is Bjerrum length; $\lb = 2\sigma = 0.7 \mathrm{nm}$ which corresponds to water the Bjerrum length in water at $T=300 \mathrm{K}$.

We use Langevin thermostat, \ie we add two additional terms to force in equation of motion
\begin{equation}
...
Integrating the Langevin motion equation for all the particles we sample total energy of the system and pressure \cite{essmann1995}.
 
\subsubsection{Monte Carlo sampling in open system}
...
\begin{equation}
    \Omega=E-TS+\sum_i\mu_i N_i\label{eq:Omega-GC}
\end{equation}
...
\begin{equation}
    S=\kB\sum_i\ln\frac{V^{N_i}}{N_i!}\label{eq:entropy}
\end{equation}
%\todo{for partiles without interactions}
It accounts for two contributions:
\begin{enumerate}
    \item the combinatorial entropy $S_{c}=-\sum_i\kB\ln N_i!$ which reflects the freedom of choice among $N$ particles
    \item the mixing entropy $S_{m}=\kB \sum_i N_i\ln V$ which reflects the freedom to place the chosen particle randomly within the simulation box. 
\end{enumerate}
$V$ is the unitless volume, \ie the volume measured in units of $\sigma^3$. 
Thus \refeq{eq:Omega-GC} turns to 
%\todo{this is not reason, moreover A.10 is already wrong as for units}
\begin{equation}
    \Omega=E-\kT\ln\prod_i\frac{V^N_i}{N_i!}+\sum_i\mu_i N_i
\label{eq: Omega_GC}
\end{equation}
The change of free energy associated with an exchange of ion pair is
\begin{equation}
    \Delta\Omega=\kT\ln\prod_i\left(V^{\xi}\frac{N_i!}{\left(N_i+\xi\right)!}\right)+\xi\mu_i+\Delta E
\label{eq: DeltaG GC}
\end{equation}
where $\xi=+1$ stands for insertion of an ion pair and $\xi=-1$ for deletion.
In general $\xi$ can be any number, but in case when  $\xi = \pm 1$ the previous equation gets simplified 
\begin{equation}
    \Delta\Omega=\kT\ln V^{2\xi} \prod_i \left(N_i+\theta(\xi)\right)^{-\xi}+\xi\mu_i+\Delta E
\label{eq: DeltaG GC_simpl}
\end{equation}
where $\theta$ is the Heaviside function, $\theta(\xi) = 1, \mathtt{if}\  \xi>0$ , $\theta(\xi) = 0, \mathtt{if}\  \xi<0$

The procedure of the Monte Carlo Sampling is the following \cite{Frenlkel2002_book}
\begin{enumerate}
	\item we propose the new configuration of the system by insertion (or deletion) of an ion pair, $\xi=\pm1$
	\item we accept the new configuration if
	\begin{equation}
        \mathcal{R}^{\xi}<e^{\Delta\Omega/\kT}=V^{2\xi} \prod_i \left(N_i+\theta(\xi)\right)^{-\xi}e^{\left(\Delta E+\xi\mu\right)/\kT}\label{eq: GC acceptance}
	\end{equation}
\end{enumerate}
...
In the same way we derive the acceptance criterion for sampling the closed system (Gibbs ensemble)
...
The free energy of the Gibbs ensemble is a sum of free energies of the gel and the external volume.

\begin{equation}
    \Omega=E\gel-TS\gel + E\out-TS\out \label{eq:Omega-GC}
\end{equation}

The entropy $S$ is given by the Boltzmann formula~\cite{Nagle2004}:
\begin{equation}
    S=\kB\sum_i\ln\frac{V\gel^{N\gel_i}}{N_i!}\label{eq:entropy}
\end{equation}
%\todo{for partiles without interactions}
It accounts for two contributions:
\begin{enumerate}
...
the Monte Carlo in the \emph{open system} box with the gel and the monovalent co- and counter-ions ions, 
