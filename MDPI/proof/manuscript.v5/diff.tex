...
%DIF DEL orig.tex   Fri Oct 14 18:25:30 2022
%DIF ADD edit.tex   Fri Oct 14 19:54:30 2022
...
\copyrightyear{2022}
%DIF 143-144c143
%DIF < \externaleditor{Academic Editor: \hl{Firstname Lastname} %MDPI: please add the Academic Editor
%DIF < }
%DIF -------
%\externaleditor{Academic Editor: \hl{Firstname Lastname}} %MDPI: please add the Academic Editor %DIF > 
%DIF -------
...
%DIF 184c183
%DIF < \Author{\hl{Mikhail Laktionov} %MDPI: Please carefully check the accuracy of names and affiliations. 
%DIF -------
\Author{Mikhail Laktionov %MDPI: Please carefully check the accuracy of names and affiliations.  %DIF > 
%DIF -------
...
%DIF 195-196c194
%DIF < \AuthorCitation{\hl{Laktionov, M.;} %MDPI: Please check all author names carefully.
%DIF <  Nová, L.; Rud, O.V.}
%DIF -------
\AuthorCitation{Laktionov, M.; Nová, L.; Rud, O.V.} %DIF > 
%DIF -------
...
\address{%
%DIF 201c199
%DIF < $^{1}$ \quad Department of Physical and Macromolecular Chemistry, Faculty of Science, Charles University in Prague, \hl{Czech Republic}%MDPI: please add the city and post code
%DIF -------
$^{1}$ \quad Department of Physical and Macromolecular Chemistry, Faculty of Science, Charles University in Prague, Prague 12800, Czech Republic %DIF > 
%DIF -------
; miklakt@gmail.com (M.L.); lucie.nova@natur.cuni.cz (L.N.)\\
%DIF 203-205c201-202
%DIF < $^{2}$ \quad St. Petersburg National Research University of Information Technologies, Mechanics and Optics, \hl{St.~Petersburg}%MDPI: please add the post code
%DIF < , Russia\\
%DIF < $^{3}$ \quad Institute of Macromolecular Compounds of Russian Academy of Sciences, \hl{Saint-Petersburg}, Russia}
%DIF -------
$^{2}$ \quad Saint-Petersburg National Research University of Information Technologies, Mechanics and Optics, Saint-Petersburg 197101, Russia\\ %DIF > 
$^{3}$ \quad Institute of Macromolecular Compounds of Russian Academy of Sciences, Saint-Petersburg 199004, Russia} %DIF > 
%DIF -------
...
%DIF > \section{\highlighting{Model and Methods} %MDPI: to AE: please note that the section title in Gels Template: 1. Introduction; 2. Results and Discussion; 3. Conclusions; 4. Materials and Methods
%DIF > \label{sec: theory}}
\section{\DIFdelbegin %DIFDELCMD < \highlighting{Model and Methods} %%%
%DIF < MDPI: to AE: please note that the section title in Gels Template: 1. Introduction; 2. Results and Discussion; 3. Conclusions; 4. Materials and Methods
%DIFDELCMD < \label{sec: theory}%%%
\DIFdelend \DIFaddbegin \DIFadd{Results and Discussion}\label{sec: RD}\DIFaddend }
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % water desalination technologies and Gibbs ensemble (https://doi.org/10.3390/nano10020293 \cite{erdos2020}).
...
In this process, the gel is supposed to be compressed/decompressed in an \emph{\DIFdelbegin %DIFDELCMD < \hl{open system} %%%
\DIFdelend \DIFaddbegin \DIFadd{open system }\DIFaddend %MDPI: Please confirm if the italics should be retained.
%DIF > Oleg Rud: Confirm, let it retain
} and in a \emph{closed system}.
The compression in the \emph{\DIFdelbegin %DIFDELCMD < \hl{open system}%%%
\DIFdelend \DIFaddbegin \DIFadd{open system}\DIFaddend } assumes that the gel is in thermodynamic equilibrium with a huge (effectively \textbf{\DIFdelbegin %DIFDELCMD < \hl{infinite}%%%
\DIFdelend \DIFaddbegin \DIFadd{infinite}\DIFaddend }) reservoir of aqueous solution \DIFaddbegin \DIFadd{\reffig{fig: open}}\DIFaddend , whereas 
the \emph{\DIFdelbegin %DIFDELCMD < \hl{closed system}%%%
\DIFdelend \DIFaddbegin \DIFadd{closed system}\DIFaddend } implies that the gel is in equilibrium with a \textbf{\DIFdelbegin %DIFDELCMD < \hl{finite} %%%
\DIFdelend \DIFaddbegin \DIFadd{finite }\DIFaddend %MDPI: Please confirm if the bold should be retained.
%DIF > Oleg Rud: Confirm, let it retain
} reservoir of an aqueous solution \DIFaddbegin \DIFadd{\reffig{fig: closed}}\DIFaddend .  
...
\begin{figure}[H]
	...
	\caption{\DIFdelbeginFL %DIFDELCMD < \hl{Diamond-like network} %%%
%DIF < MDPI: Please cite the figure in the text and ensure the first citation of each figure appears in numerical order.
 \DIFdelendFL \DIFaddbeginFL \DIFaddFL{The hydrogel compressed }\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{the simulation box}\DIFdelendFL \DIFaddbeginFL \emph{\DIFaddFL{open}} \DIFaddFL{and in }\emph{\DIFaddFL{closed}} \DIFaddFL{system}\DIFaddendFL .
		\DIFdelbeginFL \DIFdelFL{Color code represents
		the individual ion types (red: $\na$, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Red and }\DIFaddendFL blue \DIFdelbeginFL \DIFdelFL{: $\cl$, yellow: $\ca$)
		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{circles represent $\na$ }\DIFaddendFL and \DIFaddbeginFL \DIFaddFL{$\cl$ ions. $V_0$ is }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{hydrogel (grey: neutral segment ($\AH$), cyan: charged segment
		($\A$))}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{volume which gel has in free swelling equilibrium state}\DIFaddendFL .\label{fig:open and closed}}
\end{figure}
...
To sample mechanical properties of the gel and reservoir, we use \DIFdelbegin \DIFdel{MD }\DIFdelend \DIFaddbegin \DIFadd{Molecular Dynamics (MD) }\DIFaddend simulation, whereas to sample the ion distribution between the gel and a reservoir, we use \DIFdelbegin \DIFdel{MC }\DIFdelend \DIFaddbegin \DIFadd{Monte Carlo (MC) }\DIFaddend simulation.
The details of this hybrid MCMD computational technique can be found in our previous studies of polyelectrolytes in open systems \cite{Rud2020, Rud2022, Landsgesel2020a} \DIFdelbegin \DIFdel{. 
 }%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Molecular Dynamics}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We model the gel as a network of 16 linear polymer chains, each consisting of 30~monomer units.
...
%DIFDELCMD < \includegraphics[width=0.7\textwidth]{{figures/simgibbs}.pdf}%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Diamond-like network in the simulation box. Color code represents
the individual ion types (red: $\na$, blue: $\cl$, yellow: $\ca$)
and the hydrogel (gray: neutral segment ($\AH$), cyan: charged segment
($\A$)). }%DIFDELCMD < \label{fig:diamond}%%%
}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdel{Each monomer unit of the network carries a negative elementary electric charge.
...
\DIFdel{Each pair of particles interact via the truncated Lennard--Jones interaction potential, which imposes strong repulsion between all particles at short distances:
    ...
    \DIFdel{\begin{split}%DIFDELCMD < \label{eq:LJ}%%%
        V_\mathrm{LJ}(r) =
        \begin{cases}
...
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:fene}%%%
...
For our simulations, we chose $K = 10 \kT/\sigma^{2}$, $\Delta r_{\mathrm{max}} = 2 \sigma$, and $r_0 = 1.0\sigma$~\mbox{%DIFAUXCMD
\cite{Jin2007}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 
...
\DIFdel{where the first term corresponds to constant friction, with $\gamma$ being a friction coefficient,
 and the second term corresponds to random thermal force, with $\boldsymbol{\eta}_i$ being a normally distributed random vector; $\mathbf{v}_i$ is the velocity of the $i$-th particle (for details see \mbox{%DIFAUXCMD
\cite{Grest1986}}\hspace{0pt}%DIFAUXCMD
).
}%DIFDELCMD < 
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{The Monte Carlo scheme for sampling the exchange of ions in an }\emph{\DIFdel{open system}} %DIFAUXCMD
\DIFdel{is based on the formula for free energy of grand canonical ensemble $\Omega$
}\begin{displaymath}
    \DIFdel{\Omega_{\mathrm{open}}=E-TS+\sum_i\mu_i N_i%DIFDELCMD < \label{eq:Omega-GC}%%%
}\end{displaymath}%DIFAUXCMD
...
%DIFDELCMD < %%%
%DIF < Our model system represents a finite volume $V$ which exchanges particles with the bath of infinite volume.
%DIF < The infinity of the volume means, that the particle exchange between the system and the bath does not influence on the chemical potentials in the bath.
%DIF < Such ensemble is called grand canonical ensemble; the free energy in this ensemble, the Landau potential, is
\DIFdel{The entropy $S$ is given by the Boltzmann formula~\mbox{%DIFAUXCMD
\cite{Nagle2004}
}\hspace{0pt}%DIFAUXCMD
}\begin{displaymath}
    \DIFdel{S=\kB\sum_i\ln\frac{\Vgel^{N_i}}{N_i!}%DIFDELCMD < \label{eq:entropy}%%%
}\end{displaymath}%DIFAUXCMD
%DIF < \todo{for partiles without interactions}
...
%DIFDELCMD < %%%
\DIFdel{The procedure for Monte Carlo sampling is as follows: \mbox{%DIFAUXCMD
...
\DIFdel{Accept the new configuration if
        ...
        \DIFdel{\mathcal{R}^{\xi}<e^{\Delta\Omega_\mathrm{open}/\kT}=\Vgel^{2\xi} \prod_i \left(N_i+\theta(\xi)\right)^{-\xi}e^{\left(\Delta E+\xi\mu\right)/\kT}%DIFDELCMD < \label{eq: GC acceptance}%%%
	}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 	%%%
...
%DIFDELCMD < %%%
\DIFdel{In the }\emph{\DIFdel{closed system}}%DIFAUXCMD
\DIFdel{, the gel exchanges particles with the explicit finite reservoir box. 
The total number of ion species in both boxes is fixed, whereas the density of ions in the external reservoir is defined by thermodynamic equilibrium between the two subvolumes (see \reffig{fig:open and closed}b. Monte Carlo sampling of the distribution of ions between the subvolumes is performed in a way similar to that described in \mbox{%DIFAUXCMD
\cite{Panagiotopoulos1988b, Erdos2020}}\hspace{0pt}%DIFAUXCMD
. 
}%DIFDELCMD < 
...
\DIFdel{The free energy of the Gibbs ensemble is a sum of the gel's free energy and that of the external volume.
    ...
    \DIFdel{\Omega_{\mathrm{closed}}=\Egel-T\Sgel + \Eout-T\Sout %DIFDELCMD < \label{eq:Omega-GC}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdel{Using reasoning similar to that of the }\emph{\DIFdel{open system}}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{Then, the procedure for sampling is the same as that of the }\emph{\DIFdel{open system}}%DIFAUXCMD
...
We estimated the energy consumption needed for producing one liter of potable water from brine and have shown that the proposed gel compression method may compete with modern \DIFdelbegin %DIFDELCMD < \hl{desalination~technologies.} %%%
\DIFdelend \DIFaddbegin \DIFadd{desalination~technologies. }\DIFaddend %MDPI: we noticed that the supplementary file has been uploaded in submitting system. please confirm if there is a supplementary, if so, please cite it in main text and add section of supplementary materils “\supplementary{The following are available at \linksupplementary{s1}, Figure S1: title, Table S1: title, Video S1: title.} ”. Please note that the reference in supplementary should be also included in mian text.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF >  Oleg Rud: There is no need of supplementary materials anymore, since we added the method description into the main text.
}
\DIFaddbegin \section{\DIFadd{Materials and Methods}\label{sec: MM}}

\subsection{\DIFadd{Molecular Dynamics}}

\DIFadd{We model the gel as a network of 16 linear polymer chains, each consisting of 30~monomer units.
These polymer chains are connected to a diamond-like network by eight~crosslinking units.
This means there are $\Ngel=16\cdot30 + 8=488$ gel monomers in the simulation box (see \reffig{fig:diamond}).
The network is put in a simulated cubic box with volume $\Vgel$ with periodic boundary conditions, which virtually emulates an infinite polymer~network.
}\begin{figure}[H]

\includegraphics[width=0.7\textwidth]{{figures/simgibbs}.pdf}
\caption{
\DIFaddFL{Diamond-like network in the simulation box. 
Red and blue particles are the ions $\na$ and $\cl$. }\label{fig:diamond}}
\end{figure}
\DIFadd{Each monomer unit of the network carries a negative elementary electric charge.
Except for the gel monomers, the monovalent co- and counter-ions, $\cl$ and $\na$, are present in the simulation box.
The total electric charge of all the particles in the box is zero; therefore the number of $\na$ ions exceeds the number of $\cl$ ions by $\Ngel$.
}

\DIFadd{Each pair of particles interact via the truncated Lennard--Jones interaction potential, which imposes strong repulsion between all particles at short distances:
}\begin{equation}
    \DIFadd{\begin{split}\label{eq:LJ}
        V_\mathrm{LJ}(r) =
        \begin{cases}
            4 \varepsilon \left( \left(\cfrac{\sigma}{r}\right)^{12}
            - \left(\cfrac{\sigma}{r}\right)^6\right)
...
\DIFadd{The bonds connecting the gel to a network are modeled using finite extension nonlinear elastic potential (FENE)
}\begin{equation}\DIFadd{\label{eq:fene}
V_\mathrm{FENE}(r) = -\frac{1}{2} K \Delta r_\mathrm{max}^2\ln \left[ 1 - \left(\frac{r-r_0}{\Delta r_\mathrm{max}} \right)^2 \right ],
}\end{equation}
\DIFadd{where
$r$ is the distance between the bonded segments,
...
\DIFadd{All the charged particles interact via Coulomb electrostatic potential:
}\begin{equation}
    \DIFadd{V_{\mathrm{EL}}=\lb\kT \cfrac{q_{1}q_{2}}{r},
}\end{equation}
\DIFadd{where $\lb$ is Bjerrum length---$\lb = 2\sigma = 0.7~\mathrm{nm}$, which corresponds to the  Bjerrum length in water at temperature $T=300~\mathrm{K}$---and $\kB$ is the Botlzman constant.
In that sense, the solvent (water) is accounted for in the model implicitly via setting up dielectric permittivity $\epsilon=80$.
}

\DIFadd{We used the Langevin thermostat, \ie two additional terms for force in the equation of motion were added
}\begin{equation}
\DIFadd{\mathbf{f}_i =  -\gamma \mathbf{v}_i(t)+\sqrt{2\gamma \kT }\boldsymbol{\eta}_i(t),
}\end{equation}
\DIFadd{where the first term corresponds to constant friction, with $\gamma$ being a friction coefficient,
 and the second term corresponds to random thermal force, with $\boldsymbol{\eta}_i$ being a normally distributed random vector; $\mathbf{v}_i$ is the velocity of the $i$-th particle (for details see \mbox{%DIFAUXCMD
\cite{Grest1986}}\hspace{0pt}%DIFAUXCMD
).
}

\subsection{\DIFadd{Monte Carlo Sampling in an Open System}}
\DIFadd{The Monte Carlo scheme for sampling the exchange of ions in an }\emph{\DIFadd{open system}} \DIFadd{is based on the formula for free energy of grand canonical ensemble $\Omega$
}\begin{equation}
    \DIFadd{\Omega_{\mathrm{open}}=E-TS+\sum_i\mu_i N_i\label{eq:Omega-GC}
}\end{equation}
\DIFadd{where $E$ is internal energy, $T$ is temperature, $S$ is entropy, $N_i$ is the number of ions of type $i\in\{\na,\ \cl\}$, and $\mu_i$ is the corresponding chemical potential.
}

%DIF > Our model system represents a finite volume $V$ which exchanges particles with the bath of infinite volume.
%DIF > The infinity of the volume means, that the particle exchange between the system and the bath does not influence on the chemical potentials in the bath.
%DIF > Such ensemble is called grand canonical ensemble; the free energy in this ensemble, the Landau potential, is
\DIFadd{The entropy $S$ is given by the Boltzmann formula~\mbox{%DIFAUXCMD
\cite{Nagle2004}
}\hspace{0pt}%DIFAUXCMD
}\begin{equation}
    \DIFadd{S=\kB\sum_i\ln\frac{\Vgel^{N_i}}{N_i!}\label{eq:entropy}
}\end{equation}
%DIF > \todo{for partiles without interactions}
\DIFadd{which accounts for two contributions:
}\begin{enumerate}
    \item \DIFadd{The combinatorial entropy $S_{c}=-\kB\sum_i\ln N_i!$, which reflects the freedom of choice among the particles;
    }\item \DIFadd{The mixing entropy $S_{m}=\kB \sum_i N_i\ln \Vgel$, which reflects the freedom to place the chosen particle randomly within the simulation box. 
}\end{enumerate}

\DIFadd{$\Vgel$ is the unitless volume, \ie the volume measured in units of $\sigma^3$. 
Thus, the change of free energy associated with an exchange of ion pairs is
}\begin{equation}
\DIFadd{\Delta\Omega_\mathrm{open}=\kT\ln\prod_i\left(\Vgel^{\xi}\frac{N_i!}{\left(N_i+\xi\right)!}\right)+\xi\sum_i\mu_i+\Delta E
\label{eq: DeltaG GC}
}\end{equation}
\DIFadd{where $\xi$ is an algebraic number of inserted (or removed) ion pairs; in general, $\xi$ can be any number, but when $\xi = \pm 1$, which corresponds to addition or removal of only one ion pair,  \refeq{eq: DeltaG GC} gets simplified
}\begin{equation}
    \DIFadd{\Delta\Omega_\mathrm{open}=\kT\ln \Vgel^{2\xi} \prod_i \left(N_i+\theta(\xi)\right)^{-\xi}+\xi\sum_i\mu_i+\Delta E
\label{eq: DeltaG GC_simpl}
}\end{equation}
\DIFadd{where $\theta$ is the Heaviside function; $\theta(\xi) = 1$ if $\xi=+1$; $\theta(\xi) = 0$ if $\xi=-1$.
}

\DIFadd{The procedure for Monte Carlo sampling is as follows: \mbox{%DIFAUXCMD
\cite{Frenlkel2002_book}
}\hspace{0pt}%DIFAUXCMD
}\begin{enumerate}
	\item \DIFadd{Propose the new configuration of the system by insertion (or deletion) of an ion pair, $\xi=\pm1$;
	}\item \DIFadd{Accept the new configuration if
	}\begin{equation}
        \DIFadd{\mathcal{R}^{\xi}<e^{\Delta\Omega_\mathrm{open}/\kT}=\Vgel^{2\xi} \prod_i \left(N_i+\theta(\xi)\right)^{-\xi}e^{\left(\Delta E+\xi\mu\right)/\kT}\label{eq: GC acceptance}
	}\end{equation}
	\DIFadd{where $\mathcal{R}$ is a uniformly distributed random number in the range between $0$ and $1$;
	}\item \DIFadd{Then, collect the number of ions, $\Nna$ and $\Ncl$, to the samples array.
}\end{enumerate}


\subsection{\DIFadd{Monte Carlo Sampling in Closed System}}

\DIFadd{In the }\emph{\DIFadd{closed system}}\DIFadd{, the gel exchanges particles with the explicit finite reservoir box. 
The total number of ion species in both boxes is fixed, whereas the density of ions in the external reservoir is defined by thermodynamic equilibrium between the two subvolumes (see \reffig{fig:open and closed}b. Monte Carlo sampling of the distribution of ions between the subvolumes is performed in a way similar to that described in \mbox{%DIFAUXCMD
\cite{Panagiotopoulos1988b, Erdos2020}}\hspace{0pt}%DIFAUXCMD
. 
}

\DIFadd{The free energy of the Gibbs ensemble is a sum of the gel's free energy and that of the external volume.
}\begin{equation}
    \DIFadd{\Omega_{\mathrm{closed}}=\Egel-T\Sgel + \Eout-T\Sout \label{eq:Omega-GC}
}\end{equation}
\DIFadd{Using reasoning similar to that of the }\emph{\DIFadd{open system}}\DIFadd{, one can derive the change of free energy associated with ion-pair exchange 
}\begin{equation}
\DIFadd{\Delta \Omega_{\mathrm{closed}} =2 k_B T \ln \left(\frac{\Vgel}{\Vout} \right) \SUPERSCRIPT }{\DIFadd{\xi}}\DIFadd{\prod_i \left(\frac{N_{i}\gel+\theta(\xi)}{N_{i}\out+\theta(-\xi)}\right)^{-\xi} + \Delta \Egel + \Delta \Eout
}\end{equation}
\DIFadd{where $\xi$ defines the direction of the trial move, so that $\xi = -1$ when an ion pair moves from the gel to the outside volume, and  $\xi = +1$ otherwise; $\Delta \Egel$ and $\Delta \Eout$ are corresponding changes of the potential energy of the gel and the outside volumes, respectively.
}

\DIFadd{Then, the procedure for sampling is the same as that of the }\emph{\DIFadd{open system}}\DIFadd{: (1) propose the move of an ion pair; (2) accept a new state if $\mathcal{R}^{\xi}<\exp({\Delta\Omega_\mathrm{closed}/\kT}$); and (3) repeat the procedure until the desired number of samples is reached.
}

\subsection{\DIFadd{Algorithm}}
\DIFadd{As mentioned above, the whole simulation run consists of MD and MC sub-simulations of the mechanical movement of the particles and ion exchange. The algorithm is the following:
}\begin{enumerate}
\item \DIFadd{Initiate the systems to simulate: the gel of volume $\Vgel$ and the external solution of volume $\Vout$;
}\item \DIFadd{Equilibrate the system, interspersing the MD and MC stages;
}\item \DIFadd{Run the MD subsimulation and collect the observables: pressure in both volumes, $P_\mathrm{gel}$, $P_\mathrm{out}$, and distances between the nodes of the gel network. The latter is needed to estimate the autocorrelation of the MD simulation;
}\item \DIFadd{Run the MC procedure, simulating ion exchange, and collect the number of ions in both boxes, 
$\Ncl\gel$ and $\Ncl\out$; 
}\item \DIFadd{Repeat the MD and MC subsimulations until the desired length of sample arrays is reached.
}\end{enumerate}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \institutionalreview{\hl{~}}%%%
%DIF < MDPI: In this section, please add the Institutional Review Board Statement and approval number for studies involving humans or animals. Please note that the Editorial Office might ask you for further information. Please add ``The study was conducted according to the guidelines of the Declaration of Helsinki, and approved by the Institutional Review Board (or Ethics Committee) of NAME OF INSTITUTE (protocol code XXX and date of approval).'' OR ``Ethical review and approval were waived for this study, due to REASON (please provide a detailed justification).'' OR ``Not applicable'' for studies not involving humans or animals. You might also choose to exclude this statement if the study did not involve humans or animals.
\DIFdelend %DIF > \institutionalreview{\hl{~}}%MDPI: In this section, please add the Institutional Review Board Statement and approval number for studies involving humans or animals. Please note that the Editorial Office might ask you for further information. Please add ``The study was conducted according to the guidelines of the Declaration of Helsinki, and approved by the Institutional Review Board (or Ethics Committee) of NAME OF INSTITUTE (protocol code XXX and date of approval).'' OR ``Ethical review and approval were waived for this study, due to REASON (please provide a detailed justification).'' OR ``Not applicable'' for studies not involving humans or animals. You might also choose to exclude this statement if the study did not involve humans or animals.

\DIFdelbegin %DIFDELCMD < \informedconsent{\hl{~}}%%%
%DIF < MDPI: Any research article describing a study involving humans should contain this statement. Please add ``Informed consent was obtained from all subjects involved in the study.'' OR ``Patient consent was waived due to REASON (please provide a detailed justification).'' OR ``Not applicable'' for studies not involving humans. You might also choose to exclude this statement if the study did not involve humans. Written informed consent for publication must be obtained from participating patients who can be identified (including by the patients themselves). Please state ``Written informed consent has been obtained from the patient(s) to publish this paper'' if applicable.
\DIFdelend %DIF > \informedconsent{\hl{~}}%MDPI: Any research article describing a study involving humans should contain this statement. Please add ``Informed consent was obtained from all subjects involved in the study.'' OR ``Patient consent was waived due to REASON (please provide a detailed justification).'' OR ``Not applicable'' for studies not involving humans. You might also choose to exclude this statement if the study did not involve humans. Written informed consent for publication must be obtained from participating patients who can be identified (including by the patients themselves). Please state ``Written informed consent has been obtained from the patient(s) to publish this paper'' if applicable.

\DIFdelbegin %DIFDELCMD < \dataavailability{\hl{~}} %%%
%DIF < MDPI: In this section, please provide details regarding where data supporting reported results can be found, including links to publicly archived datasets analyzed or generated during the study. Please refer to suggested Data Availability Statements in section ``MDPI Research Data Policies'' at \url{https://www.mdpi.com/ethics}. You might choose to exclude this statement if the study did not report any data.
\DIFdelend %DIF > \dataavailability{\hl{~}} %MDPI: In this section, please provide details regarding where data supporting reported results can be found, including links to publicly archived datasets analyzed or generated during the study. Please refer to suggested Data Availability Statements in section ``MDPI Research Data Policies'' at \url{https://www.mdpi.com/ethics}. You might choose to exclude this statement if the study did not report any data.
...
\DIFdelbegin %DIFDELCMD < \acknowledgments{\hl{This research was supported by the Czech Science Foundation (grant 19-17847Y) and the 
%DIFDELCMD < government of the Russian Federation (grant number 14.W03.31.0022).} %MDPI: The contents in acknowledgments are the same as those in funding. please confirm if it can be deleted in acknowledgments
%DIFDELCMD <  

%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF > \acknowledgments{\hl{This research was supported by the Czech Science Foundation (grant 19-17847Y) and the 
%DIF > government of the Russian Federation (grant number 14.W03.31.0022).} %MDPI: The contents in acknowledgments are the same as those in funding. please confirm if it can be deleted in acknowledgments
 %DIF >  Confirm it can be deleted in acknowledjements
%DIF > }
...
\DIFaddbegin \DIFadd{Sandia National Laboratories (SNL): Albuquerque, NM, USA, 2003;
}\DIFaddend \newblock Technical Report; 
\DIFdelbegin %DIFDELCMD < \hl{2003}%%%
\DIFdelend \DIFaddbegin \newblock \DIFadd{https://doi.org/10.2172/809106; 
}\DIFaddend %MDPI: Please add the name of the publisher and the location of it.
.
\DIFaddbegin 


\newblock {\em {\DIFadd{Physical Chemistry}}}\DIFadd{, 9th ed.; Oxford University Press:
